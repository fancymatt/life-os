...
class HairStyleAnalyzer:
    ...
    def _load_template(self) -> str:
        ...
        custom_template_path = settings.base_dir / "data" / "tool_configs" / "hair_style_analyzer_template.md"
    ...
    async def aanalyze(
        ...
        """
        Analyze hair style (async version)
        ...
        # Perform analysis
        logger.info(f"🔍 Analyzing hair style in {image_path.name}...")
    ...
    def analyze(
        ...
        """
        Analyze hair style (synchronous wrapper)
    ...
    def save_to_preset(
        ...
        return self.preset_manager.save("hair_styles", result, display_name=name, notes=notes)
...
Examples:
  ...
  python tool.py --list
    ...
    try:
        ...
        # Print results
        logger.info("\n" + "="*70)
        logger.info("Hair Style Analysis")
        logger.info("="*70)
        logger.info(f"\nCut: {result.cut}")
        logger.info(f"Length: {result.length}")
        logger.info(f"Layers: {result.layers}")
        logger.info(f"Texture: {result.texture}")
        logger.info(f"Volume: {result.volume}")
        logger.info(f"Parting: {result.parting}")
        logger.info(f"Front Styling: {result.front_styling}")
        logger.info(f"Overall Style: {result.overall_style}")
        logger.info("\n" + "="*70)
